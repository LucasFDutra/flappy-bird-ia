...
def render_tube_pair(tubes_group, screen_size, bird_space, previous_tube_distance, tube_speed):
    previus_tube = get_previus_tube(tubes_group=tubes_group)

    bottom_tube = Tube(tube_type='bottom', speed=tube_speed, screen_size=screen_size, 
                       other_tube=None, previous_tube=previus_tube,
                       bird_space=bird_space, previous_tube_distance=previous_tube_distance
                )
    top_tube = Tube(tube_type='top', speed=tube_speed, screen_size=screen_size, 
                    other_tube=bottom_tube, previous_tube=previus_tube,
                    bird_space=bird_space, previous_tube_distance=previous_tube_distance
            )
...
def adjust_difficulty_parameters(points, previous_tube_distance, bird_space, tube_speed):
    if points % 5 == 0 and points > 0:
        previous_tube_distance = 100 if previous_tube_distance <= 100 else previous_tube_distance - 50
